...
function updateBackgroundImage(filePath) {
  try {
    ...
    // Use regular expressions to match and replace background image URLs
    const bgImageRegex = /background-image:\s*url\(['"]([^'"]+)['"]\)/g;
    
    // Replace all background image URLs with our stethoscope image
    content = content.replace(bgImageRegex, (match, url) => {
      // Only replace if it's an external URL (http/https)
      if (url.startsWith('http')) {
        return "background-image: url('/public/images/stethoscope-bg.jpg')";
      }
      return match; // Don't replace local URLs
    });
